...
const UserProfile = ({ userDetails }) => {
	return (
		<div className={styles.pageBackground}>
			<div className={styles.container}>
				...
				<div className={styles.layout}>
					<main className={styles.mainContent}>
						...
						<InfoSection title='Umiejętności' icon={null}>
							{userDetails.user_abilities.map(ab => (
								<div key={ab.id} className={styles.tagGroup}>
									<span className={styles.tag}>{ab.ability_name}</span>
								</div>
							))}
						</InfoSection>
					...
					<aside className={styles.sidebar}>
						<InfoSection title='Lokalizacja' icon={<MapPin size={18} />}>
							<p>
								{userDetails?.user_address?.street}, {userDetails?.user_address?.city.city}
							</p>
						</InfoSection>
							...
							{userDetails.user_language.map(lang => (
								<div key={lang.id} className={styles.tagGroup}>
									<span className={styles.tag}>
										{lang.language} - <strong>{lang.pivot.level}</strong>
									</span>
								</div>
							))}
						</InfoSection>
            ...
            <ul>
							{userDetails?.user_social_links.map((link,id) => (
									<li key={id}>
										<a target='_blank' href={link.url} className={styles.socialLink}>
											{link.platform_name}
										</a>
									</li>
							))}
